...
if __name__ == '__main__':
	...
	gr_train, _ 	= read_GENE_data(training_data)
	gr_test_ref, _ 	= read_GENE_data(testing_data)
	...
	for ll, level in enumerate(levels):
		...
		print(np.sum(total_sobol_est))

		gr_interp = lambda x: InterpToSpectral_obj.eval_operation_sg(multiindex_set, x)

		gr_test_approx = np.zeros(n_test_points)
		for j in range(n_test_points):
			gr_test_approx[j] = gr_interp(test_points[j, :])

		errors[ll] = np.linalg.norm(gr_test_approx - gr_test_ref, ord=2)/np.linalg.norm(gr_test_ref, ord=2)

		if ll == 1:
			print(gr_test_approx)

		print('*********************************')
	
	np.save('data/gr_test_ref.npy', gr_test_ref)
	np.save('data/gr_test_approx.npy', gr_test_approx)
